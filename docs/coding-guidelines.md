...
A change is not done when it works. It is done when it is *proven* to work at all three levels. If you add a game command, a slice, a page, a board entity, or a rule — you add all three kinds of test in the **same change**.
...
```ts
expect(() => executeGameCommand(game, { type: 'buyLandedAsset' }))
  .toThrow('There is no property awaiting purchase.');
```

Note: the engine calls `crypto.randomUUID()` and `new Date()` internally, so assert on *values and shapes*, never on whole-state snapshots or generated ids.
...
Thunks are where engine, storage, and store meet, so this is the highest-value layer to test. Use a **fresh store per test** (see §5 blockers) and assert on both the store *and* what landed in localStorage:
...
- Exhaust unions with a `switch`; when adding a union member, follow the compiler to every site, then hand-check the `Partial<Record<...>>` maps it will *not* flag.
...
**Three harness blockers must be fixed before the integration mandate is actually achievable:**

1. **`pnpm lint` is broken** — there is no ESLint configuration anywhere in the repo (`No ESLint configuration found in .../src`), despite `package.json` advertising `lint`, `lint:fix`, `check-all`, and `fix-all`. Add a flat/legacy config before relying on any lint gate.
...
Fixing these three is the prerequisite work for the policy in §1.
