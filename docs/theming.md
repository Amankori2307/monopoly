...
| `midnight`      | **Shipped as an appearance.** It was fully defined and deliberately unselectable for a long time, blocked by three things that were literals in component partials rather than palette decisions: the dice were a hardcoded white gradient with near-black pips, the modal scrims were dark translucent, and the drop shadows were ink-tinted. All three read correctly under a light palette and wrongly under a dark one, and no amount of theming could reach them. They are contract tokens now — `die-face`/`die-edge`/`die-shade`/`pip-ink`, `scrim`/`scrim-soft`, `shadow-ink`/`shadow-ink-strong` — so shipping this took one row in `APPEARANCES`, which is the claim `appearance.spec.ts` now tests. |
