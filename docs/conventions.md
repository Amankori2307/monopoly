...
| **Geometry**    | Fonts, spacing, breakpoints and board metrics are tokens in `abstracts/_tokens.scss`. Breakpoints are reached through the `below()` mixin, and a wide-but-short viewport through `landscape-compact()` — never a bare `@media` width query. Anything drawn **on the board** is sized in `cqw` (the board is a container), not `vw`: the viewport stops tracking the board as soon as the board is capped by height. |
