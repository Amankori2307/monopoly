...
Full reference: **[design-system.md](design-system.md)**, and rendered at
`#/style`. The short version:

| Axis            | Reach for                                                                                                                                                                                                                                                |
| --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Space**       | `$space-1..18` on a 4px grid — the number is the multiple, so `$space-3` is 12px. `$space-hair` (1px) sits below it on purpose.                                                                                                                          |
| **Type**        | `@include t.text(role)`. A role carries size, leading, weight, tracking, family and case **together**; that is what stops them drifting apart. It emits the `font` shorthand, so put the role BEFORE any override or the override is silently discarded. |
| **Colour**      | Theme tokens only. Never a hex, never an `rgba`. See [theming.md](theming.md).                                                                                                                                                                           |
| **Elevation**   | `$elevation-N var(--shadow-ink)` — geometry is fixed, ink is a palette decision.                                                                                                                                                                         |
| **Layers**      | `$z-*`. The header outranks every full-viewport scrim, always, so navigation is never dead while one is up. Below 10, a z-index is a component's own business.                                                                                           |
| **Motion**      | `$duration-*` and `$ease-*`, inside `@include m.motion { }` so the still state is the default.                                                                                                                                                           |
| **Corners**     | **Sharp everywhere**, from `$radius-*`. The one exception is a **physical game piece** — a pawn, a die, its pips, an owner's dot. An e2e sweep fails on anything else.                                                                                   |
| **Controls**    | One base rule per family; a variant is a modifier, never a restatement. `focus-ring()` for focus, `card-surface()` for a card.                                                                                                                           |
| **Composition** | One card component per concept, reused rather than re-styled — `SpaceCard` is the title deed, the buy decision's body, _and_ a holding in the drawer.                                                                                                    |

**Out of scope is not an exception.** `components/_board.scss` is frozen and
the guard says so once, with the reason. A genuine exception is marked inline
with `// design-system-exempt: <reason>`, and the census is ratcheted.
