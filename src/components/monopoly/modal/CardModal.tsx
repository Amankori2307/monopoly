...
const CardModal = (props: CardModalPropsType) => {
  const { card } = props;
  const SITE_INFO =
      ...
      case CARD_TYPES.SITE:
        UI = (
          <div className={style.card}>
            <p
              className={`${style.name} ${card.color} ${
                'c-' + card.textColorOnShow
              }`}
            >
              {card.name}
            </p>
            ...
            <table className={style.rentWithHouse}>
              <tbody>
                {card.rentWithHouse.map((data, index) => (
                  <tr key={index}>
        ...
        UI = (
          <div className={style.card}>
            ...
            <table className={`${style.rentWithHouse}  ${style.mt}`}>
              <tbody>
                {card.rent.map((data, index) => (
                  <tr key={index}>
                    <td className={style.text}>
                      Rent if owns {index + 1} realm rail{index ? 's' : ''}
                    </td>
                    <td className={style.houseRent}>${data}</td>
                  </tr>
                ))}
